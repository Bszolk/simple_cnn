...
X = X.reshape((X.shape[0], 28, 28))[:, np.newaxis, :, :]  # each image of size (1, 28, 28) [channel, height, width]
...
model = Model.sequential(
    Input((1, 28, 28)),
    Convolutional((1, 28, 28), (1, 5, 5), "valid", 1, 4),
    ReLU(),
    BatchNorm(),
    Pooling((4, 24, 24), (2, 2), "max"),
    Flatten((4, 12, 12)),
    Linear(64),
    ReLU(),
    Linear(10),
...
_, accuracy = model.valid(X_test, y_testOH)
